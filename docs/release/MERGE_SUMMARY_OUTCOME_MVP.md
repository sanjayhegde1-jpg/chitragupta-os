...
## Pages
- /inbox: Unified inbox with untriaged queue, SLA badges, CSV import (IndiaMART/TradeIndia), and manual social intake; creates enquiries, leads, messages, and follow-up tasks.
- /approvals: Director approval queue for WhatsApp and quote drafts; approve/reject and log outcomes to timeline.
- /lead/[id]: Lead detail with timeline, WhatsApp draft, agent draft (test mode), and quote draft actions.
...
- enquiries: id, source, sourceRef, content, contact{ name, phone, email, username }, leadId, triaged, createdAt, updatedAt
...
- approvals: id, kind, leadId, draft, status, createdAt, decidedAt?, decidedBy?
...
- Production-ready: inbox intake, approvals, quotes with approval gate, catalog uploads, dashboard metrics, Firestore security (director claim).
